...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  void initState() {
    super.initState();
    final repo = CheckoutRepository(useMockData: false);
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
        Scaffold(
          appBar: AppBar(
            title: const Text('Finalizacja zamówienia', style: TextStyle(fontWeight: FontWeight.bold)),
          ),
          ...
          Expanded(
            child: _selectedAddress != null 
              ? Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('${_selectedAddress!.firstname} ${_selectedAddress!.lastname}', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.white)),
                    Text(_selectedAddress!.address1, style: TextStyle(color: Colors.white.withOpacity(0.5))),
                    Text('${_selectedAddress!.postcode} ${_selectedAddress!.city}', style: TextStyle(color: Colors.white.withOpacity(0.5))),
                  ],
                )
  ...
  Widget _buildCarrierList(LocalizationProvider loc) {
    return FutureBuilder<List<CarrierModel>>(
      ...
      builder: (context, snapshot) {
        ...
        return Column(
          children: snapshot.data!.map((carrier) => Container(
            margin: const EdgeInsets.only(bottom: 8),
            decoration: BoxDecoration(
              color: Theme.of(context).cardTheme.color,
              borderRadius: BorderRadius.circular(12),
              border: Border.all(color: _selectedCarrier?.id == carrier.id ? Colors.orange : Colors.white.withOpacity(0.05)),
            ),
